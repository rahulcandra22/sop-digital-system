...
if (isset($_GET['action']) && $_GET['action'] == 'get_data' && isset($_GET['id'])) {
    ...
    if ($row = mysqli_fetch_assoc($q)) { echo json_encode($row); }
    else { echo json_encode(['error' => 'Data tidak ditemukan']); }
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] == 'add') {
        ...
        if (strlen($judul)<5) { setFlashMessage('danger','Judul minimal 5 karakter.'); header('Location: sop.php'); exit(); }
        if (strlen($lk)<20) { setFlashMessage('danger','Langkah kerja minimal 20 karakter.'); header('Location: sop.php'); exit(); }
    } else {
        if (empty($judul)||empty($lk)||$kid==0) { setFlashMessage('danger','Field wajib tidak boleh kosong!'); header('Location: sop.php'); exit(); }
    }
...
if (isset($_GET['delete'])) {
    ...
    $old = mysqli_fetch_assoc(mysqli_query($conn,"SELECT file_attachment FROM sop WHERE id=$id"));
    if ($old&&$old['file_attachment']&&file_exists("../assets/uploads/".$old['file_attachment'])) unlink("../assets/uploads/".$old['file_attachment']);
    if (mysqli_query($conn,"DELETE FROM sop WHERE id=$id")) setFlashMessage('success','SOP dihapus!');
    else setFlashMessage('danger','Gagal!');
    header('Location: sop.php'); exit();
}
        ...
        <div class="content-wrapper">
            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <button onclick="openModal('addModal')" class="btn btn-success"><i class="fas fa-plus"></i> Tambah SOP</button>
                </div>
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table id="sopTable">
                            ...
                            <tbody id="sopTableBody">
                                ...
                                <tr>
                                    <td><?php echo $no++; ?></td>
                                    <td><div class="sop-title-cell"><span class="sop-title-text"><?php echo htmlspecialchars($row['judul']); ?></span><span class="sop-creator-sub"><i class="fas fa-user-circle"></i><?php echo htmlspecialchars($row['creator']); ?></span></div></td>
                                    ...
                                    <td>
                                        <div class="action-btns">
                                            ...
                                            <button onclick="editSOP(<?php echo $row['id']; ?>)" class="btn btn-warning btn-sm" title="Edit"><i class="fas fa-edit"></i></button>
                                ...
                                <?php endwhile;
                                if ($total_rows==0): ?>
                                <tr><td colspan="7" style="text-align:center;padding:32px;color:var(--tmut)"><i class="fas fa-folder-open" style="font-size:24px;display:block;margin-bottom:8px;opacity:.4"></i>Belum ada SOP</td></tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
                    </div>
                    <div class="pagination-wrap" id="paginationWrap">
                        <div class="pagination-info">Menampilkan <span id="pgFrom">1</span>–<span id="pgTo">20</span> dari <span id="pgTotal">0</span> dokumen</div>
...
<div id="addModal" class="modal">
    <div class="modal-content">
        <div class="modal-header"><h3><i class="fas fa-plus"></i> Tambah Dokumen SOP</h3><button class="modal-close" onclick="closeModal('addModal')">&times;</button></div>
        <div class="modal-body">
            <div class="warn-banner"><i class="fas fa-exclamation-triangle wi"></i><div class="wt"><strong>Perhatian! Baca Sebelum Menambah Dokumen SOP!</strong>SOP hanya boleh dibuat oleh pihak yang berwenang dan telah mendapat persetujuan.</div></div>
            <form method="POST" enctype="multipart/form-data" id="formAddSOP">
                <input type="hidden" name="action" value="add">
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
                    <div class="form-group" style="margin-bottom:0">
                        <div class="form-row-label"><span class="field-name"><i class="fas fa-heading"></i> Judul SOP</span><span class="badge-req">Wajib di isi!</span></div>
                    ...
                    <div class="form-group" style="margin-bottom:0">
                        <div class="form-row-label"><span class="field-name"><i class="fas fa-folder"></i> Kategori</span><span class="badge-req">Wajib di isi!</span></div>
                ...
                <div class="form-group" style="margin-top:14px">
                    <div class="form-row-label"><span class="field-name"><i class="fas fa-align-left"></i> Deskripsi</span><span class="badge-opt">Opsional</span></div>
                    <textarea name="deskripsi" class="form-control" rows="3" placeholder="Silahkan isi tujuan dan deskripsi..."></textarea>
                </div>
                <div class="form-group">
                    <div class="form-row-label"><span class="field-name"><i class="fas fa-list-ol"></i> Langkah-langkah Kerja</span><span class="badge-req">Wajib di isi!</span></div>
                    <div class="steps-guide"><p><i class="fas fa-book-open" style="margin-right:5px"></i> Panduan Penulisan:</p><ol><li>Tulis setiap langkah secara berurutan, satu baris per langkah</li><li>Awali dengan nomor atau kata kerja aktif</li><li>Gunakan bahasa yang jelas dan mudah dipahami</li></ol></div>
                    <textarea name="langkah_kerja" id="add_langkah" class="form-control" rows="8" required placeholder="Silahkan isi langkah-langkah..."></textarea>
                    <div class="field-error" id="error-langkah" style="display:none"><i class="fas fa-exclamation-circle"></i> <span></span></div>
                </div>
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
                    <div class="form-group" style="margin-bottom:0">
                        <div class="form-row-label"><span class="field-name"><i class="fas fa-tag"></i> Status Awal</span><span class="badge-info">Perhatikan</span></div>
                        <select name="status" class="form-control"><option value="Draft">Draft</option><option value="Review">Review</option><option value="Disetujui">Disetujui</option><option value="Revisi">Revisi</option></select>
                        <div class="field-hint blue"><i class="fas fa-info-circle"></i><span>Dokumen baru umumnya dimulai dari <strong style="color:#94a3b8">Draft</strong>.</span></div>
                    </div>
                    <div class="form-group" style="margin-bottom:0">
                        <div class="form-row-label"><span class="field-name"><i class="fas fa-paperclip"></i> File Lampiran</span><span class="badge-opt">Opsional</span></div>
                ...
                <div class="confirm-box" style="margin-top:16px">
                    <label><input type="checkbox" id="confirmSOP"><span class="confirm-text"><strong>Saya menyatakan dokumen SOP ini telah mendapat persetujuan, ditulis lengkap dan akurat, serta belum tersedia di sistem.</strong></span></label>
                </div>
...
<div id="viewModal" class="modal">
    <div class="modal-content">
        <div class="modal-header"><h3><i class="fas fa-eye"></i> Detail SOP</h3><button class="modal-close" onclick="closeModal('viewModal')">&times;</button></div>
...
<div id="editModal" class="modal">
    <div class="modal-content">
        <div class="modal-header"><h3><i class="fas fa-edit"></i> Edit Dokumen SOP</h3><button class="modal-close" onclick="closeModal('editModal')">&times;</button></div>
        <div class="modal-body">
            <div class="warn-banner"><i class="fas fa-exclamation-triangle wi"></i><div class="wt"><strong>Perhatian! Mengedit SOP akan tercatat di Riwayat.</strong>Pastikan revisi ini sudah sesuai dengan standar yang berlaku.</div></div>
            <form method="POST" enctype="multipart/form-data" id="formEditSOP">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
                    <div class="form-group" style="margin-bottom:0">
                        <div class="form-row-label"><span class="field-name"><i class="fas fa-tag"></i> Status</span><span class="badge-info">Ubah Status</span></div>
                        <select name="status" id="edit_status" class="form-control"><option value="Draft">Draft</option><option value="Review">Review</option><option value="Disetujui">Disetujui</option><option value="Revisi">Revisi</option></select>
                    </div>
                    <div class="form-group" style="margin-bottom:0">
                        <div class="form-row-label"><span class="field-name"><i class="fas fa-paperclip"></i> File Lampiran</span><span class="badge-opt">Opsional</span></div>
                        <input type="file" name="file_attachment" class="form-control">
                ...
                <div class="confirm-box" style="margin-top:16px">
                    <label><input type="checkbox" id="confirmEditSOP"><span class="confirm-text"><strong>Saya menyatakan revisi SOP ini sudah benar dan saya bertanggung jawab atas perubahan data.</strong></span></label>
                </div>
                <div class="form-actions">
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    // SOP CHECKBOX (ADD)
    var chk=document.getElementById('confirmSOP'), save=document.getElementById('btnSimpanSOP');
    if(chk&&save) chk.addEventListener('change', function(){ updateSubmitButton(); });

    // SOP CHECKBOX (EDIT)
    var chkEdit=document.getElementById('confirmEditSOP'), saveEdit=document.getElementById('btnSimpanEditSOP');
    if(chkEdit&&saveEdit){
        chkEdit.addEventListener('change', function(){ saveEdit.disabled=!this.checked; saveEdit.style.opacity=this.checked?'1':'.45'; saveEdit.style.cursor=this.checked?'pointer':'not-allowed'; });
    }
    ...
    function validateAddForm(){
        ...
        if(!addKategori.value){ errorKategori.style.display='flex'; errorKategori.querySelector('span').textContent='Pilih kategori yang sesuai.'; ok=false; } else { errorKategori.style.display='none'; }
    ...
    function updateSubmitButton(){
        var c=document.getElementById('confirmSOP'), b=document.getElementById('btnSimpanSOP');
        var enabled=c.checked&&validateAddForm();
        b.disabled=!enabled; b.style.opacity=enabled?'1':'.45'; b.style.cursor=enabled?'pointer':'not-allowed';
    }
    ...
    document.getElementById('formAddSOP').addEventListener('submit', function(e){
        if(!validateAddForm()||!document.getElementById('confirmSOP').checked){ e.preventDefault(); alert('Harap periksa kembali form.'); return false; }
    });
    updateSubmitButton();
...
function editSOP(id){
    ...
    var chk=document.getElementById('confirmEditSOP'), btn=document.getElementById('btnSimpanEditSOP');
    if(chk){ chk.checked=false; btn.disabled=true; btn.style.opacity='.45'; btn.style.cursor='not-allowed'; }
    fetch(window.location.href+'?action=get_data&id='+id).then(r=>r.json()).then(data=>{
...
(function(){
    ...
    function init(){ var tbody=document.getElementById('sopTableBody'); if(!tbody) return; allRows=Array.from(tbody.querySelectorAll('tr')); filteredRows=allRows.slice(); render(); }
    function render(){
