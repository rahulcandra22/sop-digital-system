...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['action'])) {
        if ($_POST['action'] == 'add') {
            ...
            $sql = "INSERT INTO sop (judul, kategori_id, deskripsi, langkah_kerja, file_attachment, created_by) VALUES ('$judul', $kategori_id, '$deskripsi', '$langkah_kerja', '$file_attachment', $created_by)";
            ...
            $sql = "UPDATE sop SET judul='$judul', kategori_id=$kategori_id, deskripsi='$deskripsi', langkah_kerja='$langkah_kerja' $file_update WHERE id=$id";
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive"><table id="sopTable">
                            <thead><tr><th width="5%">No</th><th width="25%">Judul</th><th width="15%">Kategori</th><th width="25%">Deskripsi</th><th width="12%">Dibuat Oleh</th><th width="10%">Tanggal</th><th width="8%">Aksi</th></tr></thead>
                            <tbody>
                                <?php $no=1; while($row=mysqli_fetch_assoc($result)): ?>
                                <tr><td><?php echo $no++; ?></td><td><strong><?php echo htmlspecialchars($row['judul']); ?></strong></td><td><span class="badge"><?php echo htmlspecialchars($row['nama_kategori']); ?></span></td><td><?php echo substr(htmlspecialchars($row['deskripsi']),0,60).'...'; ?></td><td><?php echo htmlspecialchars($row['creator']); ?></td><td><?php echo date('d/m/Y', strtotime($row['created_at'])); ?></td>
                                <td>
