...
if(isset($_GET['action'])&&isset($_GET['id'])){
  ...
  if($_GET['action']=='view'){
    ...
    if($row){
      ...
      while($cat=mysqli_fetch_assoc($rcat)){$sel=($cat['id']==$row['kategori_id'])?'selected':'';echo '<option value="'.$cat['id'].'" '.$sel.'>'.htmlspecialchars($cat['nama_kategori']).'</option>';}
      echo '</select></div>';
      ...
      foreach(['Draft','Review','Disetujui','Revisi'] as $st){$sel=($row['status']==$st)?'selected':'';echo '<option value="'.$st.'" '.$sel.'>'.$st.'</option>';}
      echo '</select></div>';
      echo '<div class="form-group"><label>File Lampiran</label>';
      if($row['file_attachment'])echo '<p style="font-size:12px;margin-bottom:8px;color:var(--tmut)">File saat ini: <strong>'.htmlspecialchars($row['file_attachment']).'</strong></p>';
