                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table id="userTable">
                            ...
                            <tbody id="userTableBody">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="action-btns">
                                            <button onclick='editUser(<?php echo json_encode($row); ?>)' class="btn btn-warning btn-sm" title="Edit User">
                                                <i class="fas fa-edit"></i>
                                            </button>
                                            <?php if ($row['id'] != getUserId()): ?>
                                                <a href="?delete=<?php echo $row['id']; ?>" onclick="return confirmDelete(<?php echo $row['id']; ?>,'user')" class="btn btn-danger btn-sm" title="Hapus User">
                                                    <i class="fas fa-trash"></i>
                                                </a>
                                            <?php else: ?>
                                                <button class="btn btn-sm" disabled title="Tidak bisa hapus akun sendiri" style="background:rgba(100,116,139,.3)!important;cursor:not-allowed;opacity:.5;">
                                                    <i class="fas fa-trash"></i>
                                                </button>
                                            <?php endif; ?>
                                        </div>
                                ...
                                if ($total_rows == 0): ?>
                                <tr>
                                    <td colspan="5" style="text-align:center;padding:32px;color:var(--tmut)">
                    ...
                    <div class="pagination-wrap" id="paginationWrap">
                        <div class="pagination-info">
                            Menampilkan <span id="pgFrom">1</span>–<span id="pgTo">20</span> dari <span id="pgTotal">0</span> user
                        </div>
        ...
        <div class="modal-body">
            <div class="warn-banner">
                ...
                <div class="wt">
                    ...
                    Jangan memberikan akses Admin kepada sembarang orang.
                </div>
            </div>
            <form method="POST" id="formAddUser">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
                    <div class="form-group" style="margin-bottom:0">
                        ...
                        <input type="text" name="username" id="add_username" class="form-control" placeholder="min. 3 karakter" required>
                        <div class="field-error" id="error-add-username" style="display:none"><i class="fas fa-exclamation-circle"></i><span></span></div>
                    </div>
                    <div class="form-group" style="margin-bottom:0">
                        <label>Password</label>
                        <input type="password" name="password" id="add_password" class="form-control" placeholder="min. 6 karakter" required>
                        <div class="field-error" id="error-add-password" style="display:none"><i class="fas fa-exclamation-circle"></i><span></span></div>
                    </div>
                </div>
                <div class="form-group" style="margin-top:14px">
                    <label>Nama Lengkap</label>
                    <input type="text" name="nama_lengkap" id="add_nama" class="form-control" placeholder="min. 3 karakter" required>
                    <div class="field-error" id="error-add-nama" style="display:none"><i class="fas fa-exclamation-circle"></i><span></span></div>
                </div>
                ...
                <div class="confirm-box">
                    <label>
                        ...
                        <span class="confirm-text"><strong>Saya yakin data yang diisi sudah benar dan bertanggung jawab atas akses yang diberikan.</strong></span>
                    </label>
                </div>
        ...
        <div class="modal-body">
            <div class="warn-banner">
                ...
                <div class="wt">
                    ...
                    Mengubah password akan membuat user tersebut perlu login ulang.
                </div>
            </div>
            <form method="POST" id="formEditUser">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
                    <div class="form-group" style="margin-bottom:0">
                        ...
                        <div class="field-error" id="error-edit-username" style="display:none"><i class="fas fa-exclamation-circle"></i><span></span></div>
                    </div>
                    <div class="form-group" style="margin-bottom:0">
                        <label>Password <small style="color:var(--tmut);font-weight:400;text-transform:none">(kosongkan jika tidak diubah)</small></label>
                        <input type="password" name="password" id="edit_password" class="form-control" placeholder="min. 6 karakter">
                        <div class="field-error" id="error-edit-password" style="display:none"><i class="fas fa-exclamation-circle"></i><span></span></div>
                    </div>
                </div>
                <div class="form-group" style="margin-top:14px">
                    <label>Nama Lengkap</label>
                    <input type="text" name="nama_lengkap" id="edit_nama_lengkap" class="form-control" required>
                    <div class="field-error" id="error-edit-nama" style="display:none"><i class="fas fa-exclamation-circle"></i><span></span></div>
                </div>
                <div class="form-group">
                    <label>Role</label>
                ...
                <div class="confirm-box">
                    <label>
                        ...
                        <span class="confirm-text"><strong>Saya menyatakan perubahan data ini sudah benar dan bertanggung jawab atasnya.</strong></span>
                    </label>
                </div>
                <div class="form-actions">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // === VALIDATION: ADD USER ===
    var addUsername = document.getElementById('add_username'),
        addPassword = document.getElementById('add_password'),
        addNama     = document.getElementById('add_nama'),
        chkAdd      = document.getElementById('confirmAddUser'),
        btnAdd      = document.getElementById('btnAddUser');

    function validateAdd(){
        var ok = true;
        var eu = document.getElementById('error-add-username'),
            ep = document.getElementById('error-add-password'),
            en = document.getElementById('error-add-nama');
    ...
    function updateAdd(){
        var enabled = chkAdd.checked && validateAdd();
        btnAdd.disabled=!enabled; btnAdd.style.opacity=enabled?'1':'.45'; btnAdd.style.cursor=enabled?'pointer':'not-allowed';
    }
    [addUsername, addPassword, addNama].forEach(function(el){ el.addEventListener('input', updateAdd); });
    chkAdd.addEventListener('change', updateAdd);
    updateAdd();
    document.getElementById('formAddUser').addEventListener('submit', function(e){
        if(!validateAdd()||!chkAdd.checked){ e.preventDefault(); return false; }
    });
    ...
    var editUsername = document.getElementById('edit_username'),
        ...
        editNama     = document.getElementById('edit_nama_lengkap'),
        chkEdit      = document.getElementById('confirmEditUser'),
        btnEdit      = document.getElementById('btnEditUser');

    function validateEdit(){
        var ok = true;
        var eu = document.getElementById('error-edit-username'),
            ep = document.getElementById('error-edit-password'),
            en = document.getElementById('error-edit-nama');
    ...
    function updateEdit(){
        var enabled = chkEdit.checked && validateEdit();
        btnEdit.disabled=!enabled; btnEdit.style.opacity=enabled?'1':'.45'; btnEdit.style.cursor=enabled?'pointer':'not-allowed';
    }
    [editUsername, editPassword, editNama].forEach(function(el){ el.addEventListener('input', updateEdit); });
    chkEdit.addEventListener('change', updateEdit);
    updateEdit();
    document.getElementById('formEditUser').addEventListener('submit', function(e){
        if(!validateEdit()||!chkEdit.checked){ e.preventDefault(); return false; }
    });
});
