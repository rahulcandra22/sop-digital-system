...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action'])) {
    ...
    if (empty($nama)) { setFlashMessage('danger','Nama Kategori tidak boleh kosong!'); header('Location: kategori.php'); exit(); }
    if (strlen($nama) < 3) { setFlashMessage('danger','Nama Kategori minimal 3 karakter.'); header('Location: kategori.php'); exit(); }
    $nama = mysqli_real_escape_string($conn, $nama);
    $desk = mysqli_real_escape_string($conn, $desk);
    if ($_POST['action'] == 'add') {
        $cek = mysqli_query($conn,"SELECT id FROM categories WHERE nama_kategori = '$nama'");
        if (mysqli_num_rows($cek) > 0) { setFlashMessage('danger','Nama kategori sudah ada.'); header('Location: kategori.php'); exit(); }
...
if (isset($_GET['delete'])) {
    ...
    if (mysqli_num_rows($cek) > 0) { setFlashMessage('danger','Kategori tidak bisa dihapus karena masih digunakan oleh SOP.'); header('Location: kategori.php'); exit(); }
...
$result       = mysqli_query($conn,"SELECT c.*, (SELECT COUNT(*) FROM sop WHERE kategori_id = c.id) as jumlah_sop FROM categories c ORDER BY c.id DESC");
...
<head>
    ...
    <style>
        ...
        *,*::before,*::after{box-sizing:border-box;}
        ...
        .kat-icon{width:34px;height:34px;border-radius:8px;background:linear-gradient(135deg,rgba(59,130,246,.2),rgba(139,92,246,.2));border:1px solid rgba(59,130,246,.25);display:flex;align-items:center;justify-content:center;font-size:13px;color:#60a5fa;flex-shrink:0;}
        .kat-name{font-weight:600;color:var(--tm)!important;font-size:13.5px;}
        .sop-count{display:inline-flex;align-items:center;gap:5px;padding:4px 10px;border-radius:20px;font-size:12px;font-weight:600;background:rgba(59,130,246,.12);color:#60a5fa;border:1px solid rgba(59,130,246,.25);}
        [data-theme="light"] .sop-count{background:rgba(59,130,246,.08);color:#2563eb;border-color:rgba(59,130,246,.2);}
        ...
        .modal-content{background:var(--mbg)!important;border:1px solid var(--mbor)!important;border-radius:16px;width:92%;max-width:560px;margin:6% auto;box-shadow:0 20px 50px rgba(0,0,0,.4);max-height:88vh;overflow-y:auto;}
        ...
        .field-hint i{font-size:11px;margin-top:2px;flex-shrink:0;color:#60a5fa;}
                    ...
                    <div class="table-responsive">
                        <table id="kategoriTable">
                            <thead>
                                <tr>
                                    <th width="5%">No</th>
                                    <th width="28%">Nama Kategori</th>
                                    <th width="36%">Deskripsi</th>
                                    <th width="12%">Jumlah SOP</th>
                            ...
                            <tbody id="kategoriTableBody">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="kat-cell">
                                            <div class="kat-icon"><i class="fas fa-folder"></i></div>
                                            <span class="kat-name"><?php echo htmlspecialchars($row['nama_kategori']); ?></span>
                                        </div>
                                    </td>
                                    <td style="color:var(--tmut)!important;font-size:13px">
                                        <?php $desc = htmlspecialchars($row['deskripsi']); echo mb_strlen($desc) > 70 ? mb_substr($desc,0,70).'…' : ($desc ?: '<span style="opacity:.4;font-style:italic">—</span>'); ?>
                                    </td>
                                    <td><span class="sop-count"><i class="fas fa-file-alt" style="font-size:10px"></i><?php echo $row['jumlah_sop']; ?> SOP</span></td>
                                    <td style="font-size:12px;color:var(--tmut)!important;white-space:nowrap"><?php echo isset($row['created_at']) ? date('d/m/Y',strtotime($row['created_at'])) : '—'; ?></td>
                                    <td>
                                        <div class="action-btns">
                                            <button onclick="editKategori(<?php echo $row['id']; ?>)" class="btn btn-warning btn-sm" title="Edit"><i class="fas fa-edit"></i></button>
                                            <a href="?delete=<?php echo $row['id']; ?>" onclick="return confirmDelete(<?php echo $row['id']; ?>,'Kategori')" class="btn btn-danger btn-sm" title="Hapus"><i class="fas fa-trash"></i></a>
                                        </div>
                                    </td>
                                </tr>
                                <?php endwhile;
                                if ($total_rows == 0): ?>
                                <tr><td colspan="6" style="text-align:center;padding:32px;color:var(--tmut)"><i class="fas fa-folder-open" style="font-size:24px;display:block;margin-bottom:8px;opacity:.4"></i>Belum ada kategori</td></tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
                    </div>
                    <div class="pagination-wrap" id="paginationWrap">
                        <div class="pagination-info">Menampilkan <span id="pgFrom">1</span>–<span id="pgTo">20</span> dari <span id="pgTotal">0</span> kategori</div>
    ...
    <div class="modal-content">
        ...
        <div class="modal-body">
            <div class="warn-banner">
                ...
                <div class="wt"><strong>Perhatian Sebelum Menambah Kategori!</strong>Pastikan kategori ini belum tersedia di daftar dan benar-benar diperlukan.</div>
            </div>
            <form method="POST" id="formAddKategori">
                <input type="hidden" name="action" value="add">
                <div class="form-group">
                    <div class="form-row-label"><span class="field-name"><i class="fas fa-tag"></i> Nama Kategori</span><span class="badge-req">Wajib di isi!</span></div>
                    <input type="text" name="nama_kategori" id="add_nama" class="form-control" placeholder="Contoh: Keuangan, Operasional, SDM..." required>
                    <div class="field-error" id="error-nama" style="display:none"><i class="fas fa-exclamation-circle"></i> <span></span></div>
                </div>
                <div class="form-group">
                    ...
                    <textarea name="deskripsi" id="add_deskripsi" class="form-control" rows="4" placeholder="Jelaskan tujuan dan ruang lingkup kategori ini..."></textarea>
                    <div class="field-hint"><i class="fas fa-info-circle"></i><span>Deskripsi membantu user memahami isi kategori ini.</span></div>
                </div>
                <div class="confirm-box">
                    <label><input type="checkbox" id="confirmAdd"><span class="confirm-text"><strong>Saya menyatakan kategori ini belum tersedia di sistem dan penambahan ini telah mendapat persetujuan.</strong></span></label>
                </div>
                <div class="form-actions">
                    <button type="submit" id="btnSimpan" class="btn btn-success" disabled style="opacity:.45;cursor:not-allowed"><i class="fas fa-save"></i> Simpan</button>
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    // VALIDATION: ADD KATEGORI
    var addNama=document.getElementById('add_nama'), errorNama=document.getElementById('error-nama'),
        chk=document.getElementById('confirmAdd'), saveBtn=document.getElementById('btnSimpan');
    function validateAdd(){
        var ok=true;
        if(addNama.value.trim().length<3){ errorNama.style.display='flex'; errorNama.querySelector('span').textContent='Nama kategori minimal 3 karakter.'; ok=false; } else { errorNama.style.display='none'; }
        return ok;
    }
    function updateAdd(){ var enabled=chk.checked&&validateAdd(); saveBtn.disabled=!enabled; saveBtn.style.opacity=enabled?'1':'.45'; saveBtn.style.cursor=enabled?'pointer':'not-allowed'; }
    addNama.addEventListener('input', updateAdd);
    chk.addEventListener('change', updateAdd);
    updateAdd();
    document.getElementById('formAddKategori').addEventListener('submit', function(e){ if(!validateAdd()||!chk.checked){ e.preventDefault(); return false; } });
});
...
function searchTable(inputId, tableId){
    ...
    document.getElementById(tableId).querySelectorAll('tbody tr').forEach(function(tr){
        ...
        tr.style.display=(td&&td.textContent.toUpperCase().indexOf(val)>-1)?'':'none';
    });
}
...
(function(){
    ...
    function init(){ var tbody=document.getElementById('kategoriTableBody'); if(!tbody) return; allRows=Array.from(tbody.querySelectorAll('tr')); filteredRows=allRows.slice(); render(); }
    function render(){
