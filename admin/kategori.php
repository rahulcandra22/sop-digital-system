...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] == 'add') {
        ...
        if (mysqli_num_rows($cek) > 0) { setFlashMessage('danger', 'Nama kategori sudah ada. Gunakan nama lain.'); header('Location: kategori.php'); exit(); }
                ...
                <div class="card-body">
                            ...
                            <tbody id="kategoriTableBody">
                                ...
                                <tr>
                                    ...
                                    <td style="color:var(--tmut)!important;font-size:13px">
                                        ...
                                        echo mb_strlen($desc) > 70 ? mb_substr($desc, 0, 70).'…' : ($desc ?: '<span style="opacity:.4;font-style:italic">—</span>');
                                        ?>
                                    </td>
                                    <td>
                                        <span class="sop-count">
                                            <i class="fas fa-file-alt" style="font-size:10px"></i>
                                            <?php echo $row['jumlah_sop']; ?> SOP
                                        </span>
                                    </td>
                                    <td style="font-size:12px;color:var(--tmut)!important;white-space:nowrap">
                                        <?php echo isset($row['created_at']) ? date('d/m/Y', strtotime($row['created_at'])) : '—'; ?>
                                    </td>
                                    <td>
                                        <div class="action-btns">
                                            <button onclick="editKategori(<?php echo $row['id']; ?>)" class="btn btn-warning btn-sm" title="Edit Kategori">
                                                <i class="fas fa-edit"></i>
                                            </button>
                                            <a href="?delete=<?php echo $row['id']; ?>" onclick="return confirmDelete(<?php echo $row['id']; ?>,'Kategori')" class="btn btn-danger btn-sm" title="Hapus Kategori">
                                ...
                                if ($total_rows == 0): ?>
                                <tr>
                                    <td colspan="6" style="text-align:center;padding:32px;color:var(--tmut)">
                    ...
                    <div class="pagination-wrap" id="paginationWrap">
                        <div class="pagination-info">
                            Menampilkan <span id="pgFrom">1</span>–<span id="pgTo">20</span> dari <span id="pgTotal">0</span> kategori
                        </div>
        ...
        <div class="modal-body">
            <div class="warn-banner">
                ...
                <div class="wt">
                    ...
                    Pastikan kategori ini belum tersedia di daftar dan benar-benar diperlukan.
                </div>
            </div>
            <form method="POST" id="formAddKategori">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="nama_kategori" id="add_nama" class="form-control"
                           placeholder="Contoh: Keuangan, Operasional, SDM..." required>
                ...
                <div class="form-group">
                    ...
                    <textarea name="deskripsi" id="add_deskripsi" class="form-control" rows="4"
                              placeholder="Jelaskan tujuan dan ruang lingkup kategori ini..."></textarea>
                ...
                <div class="confirm-box">
                    <label>
                        ...
                        <span class="confirm-text">
                            <strong>Saya menyatakan kategori ini belum tersedia di sistem dan penambahan ini telah mendapat persetujuan.</strong>
                        </span>
                    </label>
                </div>
                <div class="form-actions">
                    <button type="submit" id="btnSimpan" class="btn btn-success" disabled style="opacity:.45;cursor:not-allowed">
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    var addNama  = document.getElementById('add_nama'),
        errorNama = document.getElementById('error-nama'),
        chk      = document.getElementById('confirmAdd'),
        saveBtn  = document.getElementById('btnSimpan');

    function validateAdd(){
        var ok = true;
        if(addNama.value.trim().length < 3){
            errorNama.style.display='flex'; errorNama.querySelector('span').textContent='Nama kategori minimal 3 karakter.'; ok=false;
        } else { errorNama.style.display='none'; }
        return ok;
    }
    function updateAdd(){
        var enabled = chk.checked && validateAdd();
        saveBtn.disabled=!enabled; saveBtn.style.opacity=enabled?'1':'.45'; saveBtn.style.cursor=enabled?'pointer':'not-allowed';
    }
    ...
    document.getElementById('formAddKategori').addEventListener('submit', function(e){
        if(!validateAdd()||!chk.checked){ e.preventDefault(); return false; }
    });
});
...
(function(){
    ...
    function init(){
        var tbody = document.getElementById('kategoriTableBody'); if(!tbody) return;
    ...
    function render(){
        ...
        if(pgt) pgt.textContent=total;
        if(pgf) pgf.textContent=total===0?0:start+1;
        if(pgto) pgto.textContent=end;
        var wrap=document.getElementById('pgBtns'); if(!wrap) return;
        wrap.innerHTML='';
        var prev=document.createElement('button'); prev.className='pg-btn pg-arrow';
        prev.innerHTML='<i class="fas fa-chevron-left"></i>'; prev.disabled=(currentPage===1);
        prev.onclick=function(){ if(currentPage>1){currentPage--;render();} }; wrap.appendChild(prev);
        buildPages(currentPage,totalPages).forEach(function(p){
            if(p==='...'){
                ...
                b.textContent=p;
                b.onclick=(function(pg){return function(){currentPage=pg;render();};})(p);
                wrap.appendChild(b);
            }
        });
        var next=document.createElement('button'); next.className='pg-btn pg-arrow';
        next.innerHTML='<i class="fas fa-chevron-right"></i>'; next.disabled=(currentPage===totalPages);
        next.onclick=function(){ if(currentPage<totalPages){currentPage++;render();} }; wrap.appendChild(next);
    }
