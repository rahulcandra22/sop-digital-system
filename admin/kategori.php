...
if($_SERVER['REQUEST_METHOD']=='POST'&&isset($_POST['action'])){
  ...
  if($_POST['action']=='add'){
    if(mysqli_query($conn,"INSERT INTO categories (nama_kategori,deskripsi) VALUES ('$nama','$desk')"))setFlashMessage('success','Kategori berhasil ditambahkan!');else setFlashMessage('danger','Gagal menambahkan kategori!');
  }elseif($_POST['action']=='edit'){
    $id=(int)$_POST['id'];
    if(mysqli_query($conn,"UPDATE categories SET nama_kategori='$nama',deskripsi='$desk' WHERE id=$id"))setFlashMessage('success','Kategori berhasil diupdate!');else setFlashMessage('danger','Gagal mengupdate kategori!');
  }
...
if(isset($_GET['delete'])){
  ...
  if(mysqli_query($conn,"DELETE FROM categories WHERE id=$id"))setFlashMessage('success','Kategori berhasil dihapus!');else setFlashMessage('danger','Gagal menghapus kategori!');
  header('Location: kategori.php');exit();
}

// Sesuaikan nama kolom tanggal jika di database Anda bukan 'created_at' (misal: 'tanggal')
...
<main class="main-content">
  <div class="topbar">
    <div class="topbar-left"><h2><i class="fas fa-folder" style="color:#3b82f6"></i> Manajemen Kategori</h2></div>
  ...
  <div class="content-wrapper">
    ...
    <div class="card">
      ...
      <div class="card-body">
        <div class="search-wrap"><i class="fas fa-search"></i><input type="text" id="searchInput" onkeyup="searchTable('searchInput','kategoriTable')" placeholder="Cari Kategori..."></div>
        <div class="table-responsive"><table id="kategoriTable">
          ...
          <?php endwhile;if(mysqli_num_rows($result)==0):?><tr><td colspan="5" style="text-align:center;padding:24px;color:var(--tmut)">Belum ada Kategori</td></tr><?php endif;?>
          </tbody>
        </table></div>
      </div>
    </div>
...
<div id="addModal" class="modal"><div class="modal-content">
  ...
  <div class="modal-body"><form method="POST"><input type="hidden" name="action" value="add">
    ...
    <div class="form-group"><label>Deskripsi</label><textarea name="deskripsi" class="form-control" rows="4" placeholder="Deskripsi kategori..."></textarea></div>
    <div style="display:flex;gap:10px;margin-top:20px;"><button type="submit" class="btn btn-success"><i class="fas fa-save"></i> Simpan</button><button type="button" onclick="closeModal('addModal')" class="btn btn-danger"><i class="fas fa-times"></i> Batal</button></div>
  </form></div>
</div></div>
